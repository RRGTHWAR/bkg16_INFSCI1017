package bkg16_Music;

import java.io.IOException;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;


/**
 * Servlet implementation class GenreListWS
 */
@WebServlet("/GenreListWS")
public class GenreListWS extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GenreListWS() {
        super();
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		response.setContentType("application/json");
		GenreManager gm = new GenreManager();
		String genreName = "";
		String searchType = "";
		// Below IDs allow for searching genre list by song. 
		String songID = "";
		
		// First check whether a song ID was passed and, if so, get associated genres.
		if(request.getParameter("songID") != null) {
			songID = request.getParameter("songID");
			response.getWriter().print(gm.getGenreListBySongID(songID));
		} else {  // If no song provided, can assume typical genre search.
			if (request.getParameter("genreName") != null && request.getParameter("searchType") != null){
				genreName = request.getParameter("genreName");
				searchType = request.getParameter("searchType");
			}
			// If genreName and searchType are blank, then return everything.
			response.getWriter().print(gm.getGenreList(genreName, searchType));
		}
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		doGet(request, response);
	}

}
